...
void setup() {
  ...
  if (useDMP)
  {
    ...
    // success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Quat9, 3) == ICM_20948_Stat_Ok);
  ...
  mm_settings(mainType, 0);
  setBaseInfoFeedbackMode(1);

  init_oled();
  screenLine_0 = "ugv_base_ros.git";
  screenLine_1 = "version: 0.96sj";
...
void loop() {
  ...
  if (useDMP)
  {
    ...
    if (myICM.dataReady()) {          // checks if new accel/gyro data is available
      ...
      if (myICM.status == ICM_20948_Stat_Ok) {
        ...
        // optional debug
        // Serial.print("getAGMT status: "); Serial.println((int)myICM.status);
      }
    }
